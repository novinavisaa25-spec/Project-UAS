...
@section('page-title', 'Tambah Tracking Baru')

@section('content')
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Form Tambah Tracking</h3>
    </div>
    <form action="{{ route('admin.tracking.store') }}" method="POST">
        @csrf
        <div class="card-body">
                ...
                <div class="form-group col-md-4">
                    ...
                    <select name="id_armada" id="id_armada" class="form-control @error('id_armada') is-invalid @enderror">
                        ...
                        @foreach($armadas as $armada)
                            <option value="{{ $armada->id_armada }}" {{ old('id_armada') == $armada->id_armada ? 'selected' : '' }}>
            ...
            <div class="form-row">
                <div class="form-group col-md-4">
                    ...
                    <select name="status_pengiriman" id="status_pengiriman" class="form-control @error('status_pengiriman') is-invalid @enderror">
                        ...
                        <option value="pending" {{ old('status_pengiriman') == 'pending' ? 'selected' : '' }}>Pending</option>
                        <option value="dikirim" {{ old('status_pengiriman') == 'dikirim' ? 'selected' : '' }}>Dikirim</option>
                        <option value="dalam_perjalanan" {{ old('status_pengiriman') == 'dalam_perjalanan' ? 'selected' : '' }}>Dalam Perjalanan</option>
                        <option value="sampai" {{ old('status_pengiriman') == 'sampai' ? 'selected' : '' }}>Sampai</option>
                        <option value="selesai" {{ old('status_pengiriman') == 'selesai' ? 'selected' : '' }}>Selesai</option>
                        <option value="batal" {{ old('status_pengiriman') == 'batal' ? 'selected' : '' }}>Batal</option>
                    </select>
                ...
                <div class="form-group col-md-4">
                    ...
                    <input type="date" name="tanggal_kirim" id="tanggal_kirim" class="form-control @error('tanggal_kirim') is-invalid @enderror" value="{{ old('tanggal_kirim') }}">
                    @error('tanggal_kirim')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group col-md-4">
                    ...
                    <input type="date" name="tanggal_sampai" id="tanggal_sampai" class="form-control @error('tanggal_sampai') is-invalid @enderror" value="{{ old('tanggal_sampai') }}">
            ...
            <div class="form-group">
                ...
                <input type="text" name="lokasi_terakhir" id="lokasi_terakhir" class="form-control @error('lokasi_terakhir') is-invalid @enderror" value="{{ old('lokasi_terakhir') }}" placeholder="Contoh: Bandung - Gudang">
